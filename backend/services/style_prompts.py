...
def _validate_style_data(data: dict) -> None:
    if not isinstance(data, dict):
        raise ValueError("style_prompts.json must contain a JSON object at the top level")
...
def _load_style_defs() -> dict[str, dict[str, str]]:
    try:
        ...
        raise FileNotFoundError(f"Could not find style prompt config file: {STYLE_PROMPTS_FILE}") from exc
...
def save_style_definitions(definitions: dict[str, dict[str, str]]) -> None:
    ...
    STYLE_PROMPTS_FILE.write_text(json.dumps(definitions, indent=2, ensure_ascii=False), encoding="utf-8")
    reload_style_prompts()


def get_prompt_for_style(style_name: str, base_prompt: str = "") -> str:
    attrs = STYLE_DEFS.get(style_name, STYLE_DEFS.get("Anime", next(iter(STYLE_DEFS.values()))))
