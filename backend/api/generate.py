...
async def generate_image(
    ...
    try:
        ...
        generated_base64 = await generate_styled_image(processed_base64, request)
        
        # 3. Apply post-processing VFX (Antigravity Particles + Temporal Line Refinement)
        final_base64 = apply_vfx(generated_base64, current_user.id)
        
        processing_time = round(time.time() - start_time, 2)
        
        # 3. Log the generation in the database
