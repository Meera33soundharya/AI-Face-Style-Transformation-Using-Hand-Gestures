...
def fx_grid(patch):
    """Greyscale subject under a crisp technical grid."""
    gray = cv2.cvtColor(patch, cv2.COLOR_BGR2GRAY)
    gray = cv2.convertScaleAbs(gray, alpha=1.15, beta=8)
    out = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
    out = cv2.addWeighted(out, 0.9, np.full_like(out, (60, 45, 30)), 0.1, 0)
    h, w = out.shape[:2]
    step = max(14, w // 18)
...
def fx_paper(patch):
    """Black-and-white ink outline on dotted paper."""
    h, w = patch.shape[:2]
    gray = cv2.cvtColor(patch, cv2.COLOR_BGR2GRAY)
    ...
    sheet = cv2.min(paper, edges)
    out = cv2.cvtColor(sheet, cv2.COLOR_GRAY2BGR).astype(np.float32)
    out *= np.array([0.93, 0.97, 1.0], np.float32)
    return out.astype(np.uint8)


def fx_glass(patch):
    """Apple-style glassmorphism: frosted translucent panel."""
    small, _ = _resize_for_work(patch)
...
def apply_effect(effect_fn, patch):
    """Run an effect at a capped resolution so a huge quad can't tank the FPS."""
    h, w = patch.shape[:2]
    scale = FX_MAX_DIM / max(h, w)
    if scale >= 1.0:
        return effect_fn(patch)
    small = cv2.resize(patch, (max(1, int(w * scale)), max(1, int(h * scale))))
    return cv2.resize(effect_fn(small), (w, h), interpolation=cv2.INTER_LINEAR)
...
def main():
    ...
    while True:
        ...
        if len(detected) >= 2:
            ...
            target = [left[1], left[2], right[1], right[2]]
            if smoothed is None:
        ...
        if corners is not None:
            pts_i = np.round(
                cv2.convexHull(np.array(corners, np.float32)).reshape(-1, 2)
            ).astype(np.int32)
            ...
            if x1 - x0 > 20 and y1 - y0 > 20:
                ...
                roi[sel] = processed[sel]

        if writer is not None:
            elapsed = (cv2.getTickCount() - rec_start) / cv2.getTickFrequency()
            due = min(int(elapsed * REC_FPS), frames_written + 3)
