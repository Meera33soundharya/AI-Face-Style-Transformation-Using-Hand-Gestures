...
export const EyeBlinkStudio = () => {
  ...
  const { isReady, hasFace, isBlinking, faceResult } = useEyeBlink(videoRef);
  ...
  // Blink handler: advance style → generate
  useEffect(() => {
    if (!isBlinking || !hasFace || phase === 'INIT' || phase === 'NO_FACE' || phase === 'GENERATING') return;
    const next = (styleIndexRef.current + 1) % ART_STYLES.length;
    setStyleIndex(next);
    setBlinkCount(c => c + 1);
    generateImage(next);
  }, [isBlinking, hasFace, phase, generateImage]);
      ...
      <main className="flex-1 grid md:grid-cols-2 gap-6 p-6 max-w-6xl mx-auto w-full">
        ...
        <div className="space-y-4">
          <div className="rounded-2xl overflow-hidden border border-white/10 bg-slate-900 relative aspect-video">
            ...
            {/* Blink flash overlay */}
            {isBlinking && (
              <div className="absolute inset-0 bg-white/20 pointer-events-none animate-ping rounded-2xl" />
            )}
            ...
            <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
              <span className="bg-black/70 text-xs px-3 py-1.5 rounded-full backdrop-blur">
                ...
                {phase === 'READY'      && '👁️ Blink to switch style'}
                {phase === 'GENERATING' && `⚡ Generating ${activeStyle.name}…`}
                {phase === 'SHOWING'    && `✨ ${activeStyle.emoji} ${activeStyle.name}`}
              </span>
              {hasFace && (
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-900 p-4 flex items-center gap-4">
            <div
              ...
              style={{ background: `linear-gradient(135deg, ${activeStyle.color}40, ${activeStyle.color}15)`, border: `2px solid ${activeStyle.color}` }}
            >
              {activeStyle.emoji}
            </div>
            <div className="min-w-0">
              <div className="font-bold text-lg">{activeStyle.name}</div>
              <div className="text-sm text-slate-400 truncate">{activeStyle.description}</div>
              <div className="text-xs text-slate-500 mt-0.5">Style {styleIndex + 1} of {ART_STYLES.length}</div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-900 p-4 text-sm text-slate-300 space-y-2">
            ...
            <p>👁️ <strong>Blink</strong> once to advance to the next style and generate</p>
            <p>⏱️ Wait ~1 second between blinks to avoid duplicates</p>
            <p>📸 Keep your face centered and well-lit</p>
            <p>💾 Download the result using the button below the image</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl overflow-hidden border border-white/10 bg-slate-900 relative aspect-video flex items-center justify-center">
            {phase === 'GENERATING' && (
              <div className="flex flex-col items-center gap-3 text-slate-400">
                ...
                <span className="text-sm">Generating <span className="text-white font-semibold">{activeStyle.name}</span>…</span>
            ...
            {generatedImage && phase === 'SHOWING' && (
              <div className="absolute top-3 left-3">
                <span
                  ...
                  style={{ background: `${activeStyle.color}30`, border: `1px solid ${activeStyle.color}60`, color: activeStyle.color }}
                >
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-900 p-4">
            <p className="text-xs uppercase tracking-widest text-slate-400 mb-3">All Styles</p>
            <div className="grid grid-cols-8 gap-2">
              {ART_STYLES.map((s, i) => (
                <button
                  key={s.id}
                  onClick={() => { setStyleIndex(i); if (phase === 'SHOWING') setPhase('READY'); }}
                  title={s.name}
